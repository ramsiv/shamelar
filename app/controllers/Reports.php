...
class Reports extends MY_Controller
{
    ...
    function getProductsReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($start_date) {
            ...
            $pp = "( SELECT p.date as date, product_id, purchase_id, SUM(CASE WHEN purchase_id IS NOT NULL THEN quantity ELSE 0 END) as purchasedQty, SUM(quantity_balance) as balacneQty, SUM( unit_cost * quantity_balance ) balacneValue, SUM( unit_cost * (CASE WHEN purchase_id IS NOT NULL THEN quantity ELSE 0 END) ) totalPurchase from {$this->db->dbprefix('purchase_items')} pi LEFT JOIN {$this->db->dbprefix('purchases')} p on p.id = pi.purchase_id where p.date >= '{$start_date}' and p.date < '{$end_date}' GROUP BY pi.product_id ) PCosts";
            ...
            $pp = "( SELECT p.date as date, product_id, purchase_id, SUM(CASE WHEN purchase_id IS NOT NULL THEN quantity ELSE 0 END) as purchasedQty, SUM(quantity_balance) as balacneQty, SUM( unit_cost * quantity_balance ) balacneValue, SUM( unit_cost * (CASE WHEN purchase_id IS NOT NULL THEN quantity ELSE 0 END) ) totalPurchase from {$this->db->dbprefix('purchase_items')} pi LEFT JOIN {$this->db->dbprefix('purchases')} p on p.id = pi.purchase_id GROUP BY pi.product_id ) PCosts";
